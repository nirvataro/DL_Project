...
class CommentDataset:
    def __init__(self, csv_file, device, batch_size=4):
        ...
        # parsing fields
        self.comment_parser = Field(sequential=True, use_vocab=True, lower=True, tokenize=self.tokenize,
        ...
        self.user_parser = Field(sequential=True, use_vocab=True, tokenize=lambda x: [char for char in x],
                                 init_token="<sos>", eos_token="<eos>", pad_token="<pad>", unk_token="<unk>")
        ...
        self.train_iter, self.valid_iter, self.test_iter = BucketIterator.splits((train, validation, test),
